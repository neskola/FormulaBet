...
def main(argv):
    ...
    inputfile = ""
    try:
        opts, args = getopt.getopt(argv, "harxiu:e:n:p:b:f:y:",
                                   ["user=", "email=", "name=", "password=", "bank=", "fb=", "year="])
    except getopt.GetoptError:
        print (
        "profiles.py -[a|r] [x|i] -u|--user <user name> -p|--password <password> -e|--email <email> -n|--name <name>")
        sys.exit(2)
    for opt, arg in opts:
        if opt == '-h':
